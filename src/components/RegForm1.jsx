    ...
    return(
        <div id='Form1'>
            <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={ptBR} >
                        ...
                        <TextField error={alert && !data.nacionalidade} onChange={(e) => setData((data)=>({...data,'nacionalidade':e.target.value.toUpperCase()}))} value={data.nacionalidade} style={{margin:'1%'}} variant="outlined" label='Nacionalidade' className='inputs' id='nacionalidade'></TextField>
                        <TextField error={alert && !data.endereco} onChange={(e) => setData((data)=>({...data,'endereco':e.target.value.toUpperCase()}))} value={data.endereco} style={{margin:'1%'}} variant="outlined" label='Endereço Residencial' className='EnderecoResidencial' id='EnderecoResidencial'></TextField>
                        <TextField error={alert && !data.bairro} onChange={(e) => setData((data)=>({...data,'bairro':e.target.value.toUpperCase()}))} value={data.bairro} style={{margin:'1%'}} variant="outlined" label='Bairro' className='inputs' id='Bairro'></TextField>
                        <TextField error={alert && !data.cidade} onChange={(e) => setData((data)=>({...data,'cidade':e.target.value.toUpperCase()}))} value={data.cidade} style={{margin:'1%'}} variant="outlined" label='Cidade' className='inputs' id='Cidade'></TextField>
                        <TextField error={alert && !data.ufResidencial} onChange={(e) => setData((data)=>({...data,'ufResidencial':e.target.value}))} value={data.ufResidencial} select style={{margin:'1%'}} variant="outlined" label='UF Residencial' className='inputs' id='ufResidencial'>
                        {uf.map(item=><MenuItem value={item.value}>{item.txt}</MenuItem>)}</TextField>
                        <InputMask mask="99999-999" maskChar={''} onChange={(e) => setData((data)=>({...data,'cep':e.target.value.toUpperCase()}))} value={data.cep} id='CEP' >{() => <TextField error={alert && !data.cep} style={{margin:'1%'}} variant="outlined" label='CEP' className='inputs' />}</InputMask>          
                        <TextField error={alert && !data.grayInstrucao} onChange={(e) => setData((data)=>({...data,'grauInstrucao':e.target.value.toUpperCase()}))} value={data.grauInstrucao} style={{margin:'1%'}} variant="outlined" label='Grau de Instrução' className='inputs' id='grauDeInstrucao'></TextField>
                        <TextField error={alert && !data.profissao} onChange={(e) => setData((data)=>({...data,'profissao':e.target.value.toUpperCase()}))} value={data.profissao} style={{margin:'1%'}} variant="outlined" label='Profissão/Cargo/Função' className='inputs' id='profissao'></TextField>
